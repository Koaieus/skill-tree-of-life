 ...
 *     them in a `var`, so `wa`, `wb`, `w` are doubles here too.
 ...
 *
 * Do not add -ffast-math, -march=native or anything that enables FMA
 * contraction: fusing a multiply-add here would silently break parity.
 */
...
Dictionary BladeSolverNative::simulate(
	...
	const double dt_sq = p_dt * p_dt;

	for (int step = 0; step < steps; step++) {
		const double t = (double)step * p_dt + p_dt;
		...
		for (int64_t i = 0; i < n; i++) {
			if ((double)inv_masses[i] > 0.0) {
				...
				const double sp_sq = (double)v_len_sq / dt_sq;
		...
		int iters = (int)p_base_iterations;
		if (p_velocity_iter_ref > 0.0) {
			const double max_speed = Math::sqrt(max_speed_sq);
			iters = (int)((double)p_base_iterations * (1.0 + max_speed / p_velocity_iter_ref));
		}
