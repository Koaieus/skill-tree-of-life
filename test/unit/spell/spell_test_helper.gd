...
# A `land(outcome)` helper used to live here, for tests asserting a
# crit-MULTIPLIED `HitInstance.amount`: `resolve()` decided the crit and left
# `amount` unscaled, so they had to run the applier by hand. #536 deleted it —
# `resolve()` now lands its own outcome in the shadow world it resolved
# against, so `amount` arrives scaled and a second apply would multiply twice.
# Do not add it back; if a test needs the world MUTATED, it wants the launch
# path (resolve -> capture -> rebuild -> apply live), not a raw second landing.
