...
func before_each() -> void:
	...
	# Flat board: this is a kill-sized-volley fixture, tuned CON must not ride in on the target's HP.
...
func _build_three_leaf_star(leaf_order: Array) -> Dictionary:
	...
	attacker.stat_board = TestBoards.flat_entity_board()
	_stock(attacker)
	graph.add_child(attacker)
	var hostile := Entity.new()
	hostile.faction = _NPC_FACTION
	hostile.stat_board = TestBoards.flat_entity_board()
