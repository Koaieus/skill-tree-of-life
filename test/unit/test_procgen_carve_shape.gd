...
func test_archetype_carve_shape_reaches_the_generated_node() -> void:
	...
	for n in nodes:
		if n.get_meta("archetype", &"") == tagged_archetype:
			# Compared BY VALUE, not by identity. `generate()` resolves on its own
			# copy, deep-duplicating `config.content` (it back-fills the content's
			# scalar fields, and `duplicate(true)` stops at module boundaries).
