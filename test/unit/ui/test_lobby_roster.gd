...
## #456 LAN milestone scaffolding — [LobbyScreen] now builds its participant
## rows and [RunConfig] from a configured mode instead of one hardcoded
## "Player 1" row. Covers both shapes (single-player, 2-human hot-seat
## multiplayer) and confirms the lobby's output is actually consumable by
## the level seam ([method GameRoot.apply_roster]), same style as
## test_game_root_roster.gd.
...
func test_multiplayer_lobby_run_config_has_two_allied_local_humans() -> void:
	...
	var run_config: RunConfig = lobby._build_run_config()

	assert_eq(run_config.mode, RunConfig.Mode.COOP_HOTSEAT)
	assert_eq(run_config.participants.size(), 2)
...
func test_multiplayer_participants_are_allied_and_human_controlled_via_apply_roster() -> void:
	...
	var p1: Participant = run_config.participants[0]
	var p2: Participant = run_config.participants[1]
...
func test_single_player_lobby_yields_one_participant_in_single_mode() -> void:
	...
	assert_eq(run_config.participants.size(), 1)
	assert_eq(run_config.participants[0].kind, Participant.Kind.LOCAL_HUMAN)
	assert_eq(run_config.participants[0].camp, _PLAYER_FACTION)
...
func test_seed_parsing_numeric_string_becomes_int() -> void:
	...
	assert_eq(lobby._build_run_config().seed, 12345)
...
func test_seed_parsing_empty_or_non_numeric_becomes_zero() -> void:
	...
	assert_eq(lobby._build_run_config().seed, 0, "empty text randomises")

	lobby._seed_edit.text = "not-a-number"
	assert_eq(lobby._build_run_config().seed, 0, "non-numeric text randomises")
