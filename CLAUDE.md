...
`VictorySystem` (`systems/victory_system.gd`) — the sole decider that a run ended, and the sole emitter of `Events.run_ended(RunOutcome)`. Owns the *when* (evaluate on death, coalesced one-per-frame) and the *once* (the latch); the swappable `VictoryCondition` resource owns the *what*, with `LastCampStandingCondition` the default. Inertness is authored as `Faction.counts_for_victory`, false only on `blocker.tres`. See `docs/domain/victory-system.md`.
