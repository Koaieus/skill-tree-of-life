...
`VictorySystem` (`systems/victory_system.gd`) — the sole decider that a run ended, and the sole emitter of `Events.run_ended(RunOutcome)`. Owns the *when* (evaluate on death, coalesced one-per-frame) and the *once* (the latch); the swappable `VictoryCondition` resource owns the *what*, with `LastCampStandingCondition` the default. Inertness is two separate authored `Faction` flags, false only on `blocker.tres`: `counts_for_victory` and `targeted_by_ai`. See `docs/domain/victory-system.md`.
