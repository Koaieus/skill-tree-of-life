...
## `add_modifier()`'d into a live board, so `m` has no notion of which board
## to read (#377 made `get_effective_value` take one as a parameter, but this
## preview never bound `m` to anything) — the visualizer's own `_board` (this
## panel's loaded resource) is the correct formula source, not `m`'s. This
## stays a local effective-value computation against `_board`; the row shows
## the bare contribution value ("= 5"), not a sentence — the stat name is
## already the node's own title.
