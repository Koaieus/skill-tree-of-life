...
## exists to fire a spell at an arbitrary seed, and the real targeting gate is
## what decides whether a seed is legal — so the reach is bought with a stat
## rather than bypassed. Every other targeting rule (hostile-only, min_degree
## at the source) still applies and still reports itself in the status line.
##
## Euclidean-only — [HopRangeFinder] reads `cast_range_hops`, so a hop-ranged
## spell's reach is bought with [constant CASTER_SPELL_HOPS_BONUS] instead.
## Kept large regardless: a euclidean spell's `max_distance` is scene pixels,
## and +900% still clears the playground world comfortably.
const CASTER_SPELL_RANGE_BONUS: float = 900.0

## Flat `cast_range_hops` bonus granted to the cast-from node, added onto
## [HopRangeFinder]'s `max_hops` — see [method HopRangeFinder.effective_max_hops].
...
## every Reset. Grants both reach stats so the playground covers euclidean-
## and hop-ranged spells alike.
func _grant_caster_reach() -> void:
	var range_mod := StatModifier.new()
	range_mod.stat_id = &"cast_range_distance"
	range_mod.operation = StatModifier.Operation.INCREASE
	range_mod.value = CASTER_SPELL_RANGE_BONUS
	caster_node.add_local_modifier(range_mod)

	var hops_mod := StatModifier.new()
	hops_mod.stat_id = &"cast_range_hops"
