...
## `value_at_hop(h) = base * (1 - h / range)`, clamped at 0, over the entity's
...
##
## Both [member base] and [member range] are authored explicitly, never
...
## Linear falloff: base * (1 - h/range), clamped at 0. `range <= 0` means no
## aura at all (avoids a divide-by-zero rather than reading as "infinite").
func value_at_hop(h: float) -> float:
	if range <= 0.0:
		return 0.0
	return maxf(base * (1.0 - h / range), 0.0)


## Every node reachable from [param source] within [member range] hops over
...
func values_from(source: SkillNode, mirror: GraphMirror) -> Dictionary[SkillNode, float]:
	...
	finder.max_hops = int(ceil(range))
