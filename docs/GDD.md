...
**One turn looks like:** *(structured as three phases — **CONTRACT** → **EXPAND** → **BATTLE** — split by action type. CONTRACT = deallocation only; EXPAND = allocation only; BATTLE = attacks only. One SP pool. Step 2 below is CONTRACT-then-EXPAND; step 4 is BATTLE. See `combat_system.md`.)*
