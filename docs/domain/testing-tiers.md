...
| `test_a_level_landing_during_the_dwell_keeps_counting` | `_settle()` | unit (step by hand) | `stamp()`, `release()`, `advance(min_dwell / 2)`, `stamp()` again → `is_open()` and the count reads ×2; the release timer was cancelled (`_cancel_release` is the seam, `is_open()` the observable) |
