...
## Three side-effects of allocation

Allocating a node `n` to entity `e` always does these three things, in order:
...
These three are exposed as the **`force_allocate(entity, node)`** primitive. It's the gating-free atom; everything else composes it.
...
On success: `skill_points.spend(1)` runs *before* `force_allocate`, then `allocated.emit(node, entity)` fires (inside `force_allocate`).
...
**Never call `force_allocate` from gameplay code.** It bypasses SP cost — using it for a player action quietly breaks the resource loop.
