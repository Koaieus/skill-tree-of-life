...
**Impl status:** **Built as of #299 — but may be removed entirely, see the note below.** The per-archetype off-archetype weight is `StatPack.off_phase_op_weights`, and the defensive-family exception is `constitution.tres`'s softened `{ADD_BASE:0.5, INCREASE:0.3, MULTIPLY:0.1, ADD_BONUS:0.3}` (vs. WIS/PER's `0.2/0.05/0.0/0.05`). The second half — defensive content that bypasses suppression and the cost cap outright — is the `Role.DEFENSIVE` pools. `test_constitution.gd` pins both.

#299 moved those DEFENSIVE pools out of the deleted `defensive.tres` and into `constitution.tres`, so both halves now live in one pack. That was a pure authoring-home change: `flatten_for_phase` filters per-`TierPool`, so generated content was byte-identical across the move.

**Forward-pointer — D-12 may be revised away.** There is an open proposal to **reject off-archetype rolling entirely**, on the grounds that D-12's whole rationale ("a hard migration would starve every other archetype of survivability") only held while defensive modifiers had no natural home — and CON is now that home. If that lands, `off_phase_op_weights`, the off-attribute cost cap and the `&"off"` phase all disappear, taking this decision with them. That proposal is **#319**. So: if you find this marked "built" and the field gone, the doc did not drift — read #319 first.
