...
- **Shell-swing (melee option, new):** the Halo can select part of its thorny shell as a phantom blade and **swing it offensively** — the same spike-ringed nodes that punish incoming melee (via Thorns, `armor.tres` / `min_damage_taken.tres` — unrelated to `spikes`, see below) now drive through what they sweep, via each node's own Spikes-addon-granted `blade_damage`. A closed shell ring is also a *cycle*, so a selected arc floods into a face (area damage) and the ring's own triangulation lends rigidity. Turns the defensive ring into an occasional offensive sweep. *(The "since thorns = spikes (sharpness)" framing this line used to open with never shipped — #778 resolved `spikes` as its own node-local pop-budget stat, separate from Thorns; see `combat_system.md`'s Thorns=spikes `FLAG` resolution.)*
- **Spike-ring / wall (new, OPEN on the collision model):** if the **Spikes** mechanic adopts the *structural* defensive model (spikes attack an incoming phantom blade's edges/faces, not just deal counter-damage — see `skill_node_addons.md` Spikes and `combat_system.md` Thorns=spikes `FLAG`), the Halo's shell aura granting spikes becomes a literal **wall against melee blades**: a blade that collides with the ring can be **de-rigidified** (a braced cleaver pops an edge and floppers into a whip mid-swing). **Counterplay is preserved** — a well-placed **ranged** attack knocks out a shell node and **distorts the wall** (opens a gap); ranged stays the answer to a spike ring. *(Proposal, OPEN: the spikes aura targets **any** node at shell distance, including enemy-owned — an enemy that allocates onto your ring gets spiked by your aura, intruding = self-harm. Positional, not ownership-gated. Gated behind the collision-model decision.)*
