...
| A4.3 | `systems/allocation_system.gd:427` | `_real_neighbours` hand-rolls an edge walk; `graph.get_neighbours()` is the cached index. Keep the `n != node` self-loop filter. | systems #17 | **done** |
| A4.4 | `systems/allocation_system.gd:489,498` + `ui/.../node_highlight_overlay.gd:65` | `can_allocate` is O(N+E) and the highlight overlay calls it per node → O(N·(N+E)) per repaint, re-fired on every alloc/dealloc/SP change. `_has_any_owned_node` → `entity.navigator.get_mirrored_nodes().is_empty()`; adjacency → `get_neighbours`. | systems #4 | **done, with a correction to the prescription.** The adjacency half went to `get_neighbours` as written. The `_has_any_owned_node` half did NOT become `navigator.get_mirrored_nodes().is_empty()` outright: an EMPTY mirror is not proof of no ownership (a direct `node.owned_by = X` leaves it stale — `test_intent_dispatch` does exactly that and went red), and trusting it drops `can_allocate`'s adjacency requirement, i.e. **the gate fails OPEN**. Only the positive answer takes the fast path; the negative falls back to the scan. |
...
| A4.6 | `archetypes/archetype.gd:24` | `color` getter does `StatRegistry.get_def(primary_stat).tint_color` unguarded on a `@tool` script — a typo'd id takes the editor down on a property *read*. Null-guard + `push_warning`. | graph-core #12 | **done** |
| A4.7 | `attack/range_finder/range_finder.gd:54` | Base `gather()` writes `0.0` for every node while its docstring promises the distance that feeds `DistanceScale`. Make it `@abstract` (both concretes override) or return `-1.0`. | attack #10 | **done** — `-1.0`, not `@abstract`; nothing instantiates the base, so the sentinel matches `max_reach()`'s existing "unknown" convention without touching the type. |
| A4.8 | `skill_node/skill_node.gd:1409` | `can_attach_addon` calls `get_addons().size()` — duplicates the whole ledger to count it, per candidate node in the #406 UI. Use `_addons.size()`. | skill-node #20 | **done** |
| A4.9 | `systems/loot_system.gd:163-165` | `_removed_this_attack` is cleared only under `if battle_system != null` but read unconditionally, so an unset export makes kill XP count earlier attacks' territory at bonus rate. Also clear in `_on_entity_dying` after payout. | systems #9 | **done** — clears per victim after payout, and an unset `battle_system` now `push_warning`s instead of silently disabling the #182 trickle. |
| A4.10 | `ui/vfx/coordinator/magic_bounce_coordinator.gd:189` | `_play_cancel` writes `global_position` on an untyped `Node`; a `Control`-rooted override crashes. Cast `as Node2D` + guard. | vfx #22 | **done** |
| A4.11 | `effects/core_aura.gd:27` | `@export var range: float` shadows GDScript's built-in `range()` in the class and every subclass. Rename to `hop_range`. | vfx #21 | **done** — the shadowing was LIVE, not theoretical: `core_aura.gd:54` read `ceil(range)`, which only compiled because the member shadowed the builtin. Three test files and `tools/balance/balance_scenarios.gd` wrote `aura.range`; GDScript accepts a write to a nonexistent property at parse time, so only the suite caught them. |
