@tool
class_name BattlePhaseBody
extends PhaseBodyBase

## BATTLE-phase body: dispatches on the active [AttackPlan]'s mode (melee /
## ranged / magic) and renders the relevant plan summary plus a live damage
## preview. Subscribes to [BattleSystem.attack_plan_changed] +
## [BattleSystem.attack_plan_state_changed] so the body re-renders on every
## plan mutation (pivot pick, blade extension, spell swap, target click).
...
func bind(p_entity: Entity, p_battle_system: BattleSystem) -> void:
	_disconnect_system()
	super(p_entity, p_battle_system)
