@tool
class_name ResonatorGhostLoopBody
extends Node2D

## Resonator's SELF_LOOP body (#678) — "self-loops are explicitly weaponised
## by this spell", given the SAME ghost-twin treatment Reverberator uses: a
## second [BoltPacket] head sampled ~0.1 behind the lead's own `t`, both
## riding whatever [SelfLoopPath] the coordinator evaluates for the whole
## [Projectile]. Structurally identical to
## `reverberator_ghost_loop_body.gd` — duplicated rather than shared because
## each spell owns its own file namespace under this swarm unit (never a
## cross-spell file neither issue's ownership lists), not because the logic
## actually differs.
##
## Drops into [code]ResonatorArrivalVisual[/code]-shaped composition exactly
## like a bare [BoltPacket] does for JUMP/EDGE — implements the same duck
## contract (`tint`, `_on_launch`/`_on_progress(t)`/`_on_arrival()`/
## `_on_crit(tier)`/`_on_context(entry)`, `finished`).
##
## The ghost is [member Node2D.top_level] (world-space, like [BoltBody]'s own
## trail segments) because this node's parent is the flying, moving
## [Projectile] — a non-top_level ghost would inherit the parent's transform
## and sit glued to the lead instead of trailing it. Position is sampled from
## an in-memory `(t, position)` history rather than re-evaluating a path
## resource — this body never sees [ProjectilePath] or the raw origin/target
## [Vector2]s, only the `t` [Projectile] feeds it and its own (inherited)
## [member Node2D.global_position] each frame.
