@tool
class_name DegreeRanker
extends NodeRanker

## ENTITY degree of the candidate — degree within the candidate's own owner's
## territory (the induced subgraph of their owner), matching
## [RankThresholdFilter] and the game-wide default (see
## [code].claude/rules/degree.md[/code]). This is the ranker to reach for
## unless "most connected on the whole board regardless of ownership" is
## deliberately the metric — that's [GraphDegreeRanker].
##
## Read against THIS cast's world ([method LandingContext.entity_degree_of],
## #860): ranking must see the cast's own earlier deallocations, not
## pre-cast territory.


func score(node: SkillNode, lctx: LandingContext) -> float:
	if lctx.cast.graph == null or node == null:
		return 0.0
	return float(lctx.entity_degree_of(node))


func get_description() -> String:
	return "degree"
