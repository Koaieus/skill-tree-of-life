...
## - Behind-the-disk read: [method add_rig] parks a depth-only disc at z=0 per
##   rig (writes depth, adds nothing to colour, drawn first among transparents)
##   so the far half of a HOLO_GLASS ring inside the disk radius is culled and
##   the 2D disk shows through the transparent background.
...
## The one GimbalWorld of `host`'s viewport: found by group, else instanced as
## a sibling of `host` (same canvas, so the same camera transform applies).
...
## under it; returns the holder to move/free later.
