extends SceneTree
## Bakes every mapped spell icon into a CARVE LUT and packs the set into the
## [CarveAtlas] the live shader samples (#247). Driven by `mise run icons:update`
## as the last stage of the spell-icon lifecycle (SVG -> recolor -> rasterize ->
## import -> BAKE -> PACK), so the atlas can't go stale against the icons:
##
##     godot --headless --script res://tools/bake_carve_atlas.gd
##
## Emits three things, all committed:
##   - `assets/emblem_luts/<name>.png` — the per-icon baked LUT (#246's format).
##     Reviewable per icon, and the asset an author points a
##     [member TextureCarveShape.baked_lut] at.
##   - `assets/emblem_luts/carve_atlas.png` (+ `.import`) — the same LUTs stacked
##     vertically, imported as a [CompressedTexture2DArray].
...
## keeps it lossless RGBA8. Verified: every imported layer is byte-identical to
## [method TextureCarveShape.bake_lut]'s output.
...
const ICON_DIR := "res://assets/icons/spells/"
...
func _initialize() -> void:
	...
	for i in names.size():
		...
		var source: Texture2D = load(ICON_DIR + name + ".png")
		if source == null:
			printerr("bake_carve_atlas: missing icon ", name, " — run `mise run icons:update` first")
			quit(1)
			return
		var lut := TextureCarveShape.bake_lut(source).get_image()
		lut.convert(Image.FORMAT_RGBA8)
		var lut_path := LUT_DIR + name + ".png"
		lut.save_png(lut_path)
