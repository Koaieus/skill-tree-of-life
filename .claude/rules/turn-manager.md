...
A pool sitting **exactly at its cap from a raw write** (rather than having crossed it via `replenish`) is a trap for a fixture: `PoolStat.set_current`'s `is_equal_approx(clamped, current): return` guard makes every later `replenish` onto it a no-op forever, since the clamped target equals the already-there value — it never re-crosses. Get an entity "ready" in a test by ticking it up to and past the cap (`TurnManager.tick()`), never by writing `initiative.current = cap` directly.

### `order_ready` — the ONE tie-break, shared by the live path and the forecast (#909)

`TurnManager.order_ready(entries: Array[Dictionary], last_key: int) -> Array[Dictionary]` is a **static, pure** function over plain records — `{key: int (entity_id), current: float}` — never `Entity`, because `forecast` (below) simulates future `current` on copies and can't hand it an Entity-reading comparator. Tie-break, in order: carried `current` desc (more overshoot wins) → the entry whose `key == last_key` (the just-acted entity) goes last → `key` ascending — `Entity.entity_id`, the spawn-order index `Graph._mint_entity_id` mints on entry to `entities_container`, never instance id or tree position. `TurnManager.NO_LAST_KEY` (`0`) is the sentinel for "no just-acted entity" — safe because minted ids start at 1. `_tick_until_ready` builds its entries from the live `READY_GROUP` and calls this instead of carrying its own comparator — **never a second copy of the tie-break** (no-parallel-mirrors).

### `forecast(n)` — dry-run for the HUD strip, never `tick()`

`TurnManager.forecast(n: int) -> Array[Entity]` dry-runs the tick-until-ready loop over **copies** of `(current, cap, speed)` for every live entity, starting from the live pools and `current_entity` as the first `last`, calling `order_ready` exactly as the live path does. Returns the next `n` actors (an entity may repeat); mutates no `PoolStat` — it never calls `tick()` or writes back to a real pool, only to its own `Dictionary` copies. `forecast_changed` fires after `turn_started`, after `turn_ended`, and whenever any live entity's `initiative.current_changed` or `initiative_speed.value_changed` fires; the binding is re-walked across `Entity.GROUP` on every spawn (`SceneTree.node_added`) and death (`Events.entity_died`) so a subscriber (`#910`'s strip) needs exactly one signal, never a per-entity one.
