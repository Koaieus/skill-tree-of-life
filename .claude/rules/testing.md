...
- **The theory-free version of that check: the tracked test-file count and GUT's `scripts` count must be EQUAL.** Reading "did the total go UP" needs a remembered baseline, which goes stale the moment master moves under you. This doesn't:
  ...
  grep -oE '[0-9]+ scripts' .godot/gut-last.log | tail -1
  ```
  `.gutconfig.json` scans `res://test/unit/` with `include_subdirs` and GUT collects `test_*.gd` only, so those two numbers are the same number counted twice. **Any shortfall means a file was skipped** — parse error, stale class cache, a breaking rename landed underneath you — without needing a theory of *why* first. Two traps: bare `git ls-files 'test/**/*.gd'` gives a much larger count (non-`test_` helpers and fixtures — not a problem, don't chase it), and the glob must stay pinned to `test/unit/` because that is all `dirs` scans; a `test_*.gd` added under `test/perf/` would inflate the git side and read as a false skip. Verified on 424/424, 2026-09-08.
