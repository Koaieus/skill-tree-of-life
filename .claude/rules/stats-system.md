...
Eight FLOAT board stats, one pair per DoT family: `poison_potency`, `corruption_potency`, `curse_potency`, `wither_potency` (default 1.0, attacker-side) and `poison_resistance`, `corruption_resistance`, `curse_resistance`, `wither_resistance` (default 0.0, a fraction, defender-side, read **node-locally** via `get_local_value` like armor). A `StatusDef` names its pair via `potency_stat_id` / `resistance_stat_id` (blank = unscaled); `StatusInstance.land_on` folds `power × potency(attacker) × (1 − resistance(node))` in **once** (`power_resolved`), and `AttackRecord.rebuild` marks the hit resolved so a peer replays the landed number flat. Resistance reduces stacks *incurred*, never decay. Procgen (#974): the four resistances are universal in `constitution.tres`, ADD_BASE (unit 0.05), T2–T4 only — a T1 node never rolls one; each potency lives in its attribute's pool — poison DEX, corruption STR, curse WIS, wither CON — **INCREASE only** (unit 7 → +7/+21/+49%); a `+1` ADD_BASE on potency is a doubling and is reserved for rare keystones. See `docs/design/damage_over_time.md`.
