...
- **`duplicate(true)` does NOT clone a *live* board — use `StatBoard.clone_live()` (#498).** Exports only, so every `Stat.bins` comes back fresh/empty and `_extra_stats` (every dynamically-minted stat, `node_health` among them) is missing outright. No error: the clone simply reads as an unmodified board. A bare `duplicate(true)` is still right for cloning a **virgin template** you then build up — `Entity._ready`, `SkillNode._init_node_board`, `EffectContext.grant` — which is why this went unnoticed until a combat shadow needed to clone an already-modified one. `clone_live` copies `multipliers`/`winning_set`, `_modifiers` and `_last_contrib` one level deep (the Arrays are the clone's own; the `StatModifier`s are shared), which is safe because a modifier is stateless and may live on N boards at once (#377).
- **A bin tally is only meaningful next to the `_modifiers` list it was folded from — never copy one without the other.** `add_modifier` ends in `_resync_bins_if_trivial()`, which *wipes* the bins and rebuilds from `_modifiers` whenever that list holds 0 or 1 entries. So a board carrying bins but no modifier list throws its whole tally away on the very next add (measured: 40 STR → 20 after a +10) and mis-subtracts on the next remove. That was `clone_live`'s bug until `1f95076`; the carry now lives in `Stat.adopt_modifier_list(src)`. **How to apply:** any new path that populates bins directly must populate `_modifiers` too, or the board is read-only-and-doesn't-say-so.
- **A cloned board computes but does not *react*, deliberately (#506 open).** Formula binding is per-board signal wiring `duplicate()` can't carry, and rebuilding it with `bind_modifier` in a loop is *wrong*, not merely missing: the modifiers are shared instances, so the clone's `value_changed` would `emit_changed()` on a modifier the **live** board is also subscribed to (recompute + notification storms on the real world from a shadow), and the connected `Callable` holds a strong ref that keeps every shadow `Stat` alive as long as the shared modifier does. Reacting needs per-clone copies of the *formula-bearing* modifiers, or a read-through design that clones nothing. See #506.
- **Cloning a board costs `Resource.duplicate(true)`, not bins.** Measured 2026-08-21 (`test/perf/bench_combat_snapshot.gd`, RX 7900 XTX, headless): entity board **420 µs**, node board **31 µs**, of which the bin copy is *below microsecond resolution* on both. So one `EntityCombat.snapshot()` at 200 owned is ~10 ms — 1.5 frames of the 144Hz budget — and "copy the bins more cleverly" optimises nothing. If per-slice cost ever bites, the lever is composing `ModifierBins.compute(base, [live.bins, …, overlay.bins])` (it is already N-source) instead of copying.
