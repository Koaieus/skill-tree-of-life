...
- **Never populate bins without the `_modifiers` list they were folded from** — `_resync_bins_if_trivial()` wipes the tally the moment that list holds ≤1 entry (40 STR → 20 after a +10). `Stat.adopt_modifier_list` is the carry. A cloned board also **computes but does not react** (#506), and cloning costs `duplicate(true)` — 420 µs entity / 31 µs node — while the bin copy is unmeasurable. All three, with the snapshot ramp: `docs/domain/stat-board-classes.md`.
