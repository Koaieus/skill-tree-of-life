...
Eight FLOAT board stats, one pair per DoT family: `poison_potency`, `corruption_potency`, `curse_potency`, `wither_potency` (default 1.0, attacker-side) and `poison_resistance`, `corruption_resistance`, `curse_resistance`, `wither_resistance` (default 0.0, a fraction, defender-side, read **node-locally** via `get_local_value` like armor). A `StatusDef` names its pair via `potency_stat_id` / `resistance_stat_id` (blank = unscaled); `StatusInstance.land_on` folds `power × potency(attacker) × (1 − resistance(node))` in **once** (`power_resolved`), and `AttackRecord.rebuild` marks the hit resolved so a peer replays the landed number flat. Resistance reduces stacks *incurred*, never decay. Procgen (#974, #1058, #1059): each family's pair lives in one attribute pack — poison DEX, corruption STR, curse CON, wither INT; the resistance rolls on **blessed** nodes only, ADD_BASE (unit 0.05), T2–T4 — a T1 node never rolls one; the potency rolls on **blighted** nodes only, **INCREASE only** (unit 7 → +7/+21/+49%); a `+1` ADD_BASE on potency is a doubling and is reserved for rare keystones. See `docs/design/damage_over_time.md`.

**DoT extra stacks + blindness pair (#1089, hub #1060)** — seven FLOAT board stats, entity-scope. Five flat attacker-side extras, default 0: `poison_stacks_per_hit`, `corruption_stacks_per_hit`, `curse_stacks_per_hit`, `wither_stacks_per_hit` and the umbrella `dot_stacks_per_hit`. A `StatusDef` lists the ones it reads in `extra_stacks_stat_ids` (the four DoT defs: their own + the umbrella; blindness / armor-break: none), and `land_on` sums them into the per-hit power **before** potency: `(per_hit + Σ extras) × potency × (1 − resistance)`. The umbrella lands more stacks, never scales damage across families — potency stays per family. `blindness_potency` (default 1.0) / `blindness_resistance` (default 0.0, node-local) are the blindness def's pair, the same shape as the DoT pairs. No procgen content yet (#1061's children); no falloff/duration stats (the decay shape is per def).
